...
const handleDownloadReceipt=async(data:any)=>{
  try {
    if (!data.invoice_uuid) {
      alert("Invoice UUID not found for this purchase.");
      return;
    }
    const res = await fetch(`${API_BASE_URL}/public/slot-booking/receipt/${data.invoice_uuid}`);
...
function ConfirmAlert({title,message,onConfirm,onCancel}:{title:string,message:string,onConfirm:()=>void,onCancel:()=>void}){
  return(
    <div style={{position:"fixed",inset:0,zIndex:4000,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(15,23,42,.6)",backdropFilter:"blur(6px)",animation:"fadeIn .2s"}}>
      <div style={{background:"#fff",borderRadius:0,boxShadow:"0 20px 40px rgba(0,0,0,.2)",width:"90%",maxWidth:400,padding:24,textAlign:"center",animation:"slideUp .2s"}}>
        <div style={{width:50,height:50,borderRadius:0,background:"#eff6ff",color:"#3b82f6",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px"}}><Icon name="download" size={24}/></div>
        ...
        <div style={{display:"flex",gap:12,justifyContent:"center"}}>
          <button onClick={onCancel} style={{padding:"10px 20px",borderRadius:0,border:"1.5px solid #e2e8f0",background:"#fff",color:"#475569",fontWeight:700,cursor:"pointer",flex:1}}>Cancel</button>
          <button onClick={onConfirm} className="btn-primary" style={{padding:"10px 20px",borderRadius:0,flex:1}}>Download</button>
        </div>
      </div>
    </div>
  )
...
function InstituteBankDetails({onDetailsChange}:{onDetailsChange:(d:any)=>void}){
  ...
  if(editing) return(
    <div style={{background:"#fff",border:"1px solid #e2e8f0",padding:12,borderRadius:0,fontSize:13}}>
      <div style={{fontWeight:800,color:"#0f172a",marginBottom:8,fontSize:12,textTransform:"uppercase"}}>Edit Bank Details</div>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        <input value={details.bankName} onChange={e=>setDetails({...details,bankName:e.target.value})} placeholder="Bank Name" style={{padding:8,border:"1px solid #e2e8f0",borderRadius:0,fontSize:12,width:"100%",boxSizing:"border-box",outline:"none"}}/>
        <input value={details.accNo} onChange={e=>setDetails({...details,accNo:e.target.value})} placeholder="Account Number" style={{padding:8,border:"1px solid #e2e8f0",borderRadius:0,fontSize:12,width:"100%",boxSizing:"border-box",outline:"none"}}/>
        <input value={details.ifsc} onChange={e=>setDetails({...details,ifsc:e.target.value})} placeholder="IFSC Code" style={{padding:8,border:"1px solid #e2e8f0",borderRadius:0,fontSize:12,width:"100%",boxSizing:"border-box",outline:"none"}}/>
        <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:4}}>
          <button type="button" onClick={()=>setEditing(false)} style={{padding:"6px 12px",borderRadius:0,border:"1px solid #e2e8f0",background:"#fff",fontSize:12,cursor:"pointer",color:"#475569"}}>Cancel</button>
          <button type="button" onClick={handleSave} style={{padding:"6px 12px",borderRadius:0,border:"none",background:"#6366f1",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer"}}>Save</button>
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:12,width:"100%"}}>
      ...
      {method==="Bank Transfer"&&(
        <div style={{background:"#f8fafc",padding:16,borderRadius:0,border:"1px solid #e2e8f0",display:"flex",flexDirection:"column",gap:12}}>
      ...
      {method==="Generate Invoice Link (Pending)"&&(
        <div style={{background:"#f0fdf4",padding:16,borderRadius:0,border:"1px solid #dcfce7",color:"#16a34a",fontSize:13,fontWeight:600,display:"flex",alignItems:"center",gap:8}}>
      ...
      <div style={{width:"100%",maxWidth:560,background:"#fff",borderRadius:0,boxShadow:"0 24px 60px rgba(0,0,0,.2)",overflow:"hidden",display:"flex",flexDirection:"column",animation:"slideUp .3s cubic-bezier(.16,1,.3,1)"}}>
        ...
        <div style={{padding:"20px 22px",overflowY:"auto"}}>
          <form id="reg-form" onSubmit={submit}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 14px"}} className="reg-grid">
        ...
        <div style={{padding:"14px 22px",borderTop:"1px solid #f1f5f9",display:"flex",justifyContent:"flex-end",gap:10,background:"#fafbfc"}}>
          <button type="button" onClick={onClose} style={{padding:"10px 20px",borderRadius:0,border:"1.5px solid #e2e8f0",background:"#fff",color:"#475569",fontWeight:600,fontSize:13,cursor:"pointer"}}>Cancel</button>
          ...
          {loading?<div style={{textAlign:"center",padding:40,color:"#94a3b8"}}>Loading schedule...</div>: (
            <>
              ...
              <div style={{display:"flex",gap:12,marginBottom:24,flexWrap:"wrap"}}>
                ...
                <div style={{flex:"1 1 140px",background:"#f0fdf4",padding:16,borderRadius:0,border:"1px solid #bbf7d0"}}>
                ...
                <div style={{flex:"1 1 140px",background:"#fef2f2",padding:16,borderRadius:0,border:"1px solid #fecaca"}}>
            ...
            <>
              ...
              <div style={{display:"flex",gap:12,marginBottom:24,flexWrap:"wrap"}}>
                ...
                {data.refunded_amount>0&&(
                  <div style={{flex:"1 1 140px",background:"#f8fafc",padding:16,borderRadius:0,border:"1px solid #94a3b8"}}>
              ...
              {data.due_amount>0&&(
                <div style={{background:"#f8fafc",padding:"20px",border:"1px solid #e2e8f0",borderRadius:0,marginBottom:24}}>
                      ...
                      <div style={{textAlign:"right",display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                        ...
                        {t.status==="pending"&&(
                          <button onClick={()=>{
                            ...
                            .then(()=>fetchDetail());
                          }} style={{background:"#16a34a",color:"#fff",border:"none",padding:"4px 10px",borderRadius:0,fontSize:11,fontWeight:600,cursor:"pointer",marginTop:4}}>Approve</button>
                        )}
                      </div>
                    </div>
                  ))
      ...
      {showLinkModal&&(
        <div style={{position:"absolute",inset:0,zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(15,23,42,.55)",backdropFilter:"blur(4px)"}}>
          {(()=>{
            const isPending = form.method === "Generate Invoice Link (Pending)";
            return (
              <div style={{width:"92%",maxWidth:460,background:"#fff",borderRadius:0,boxShadow:"0 24px 60px rgba(0,0,0,.25)",overflow:"hidden",animation:"slideUp .25s cubic-bezier(.16,1,.3,1)"}}>
                ...
                <div style={{
                  ...
                  <div style={{
                    width:60,height:60,borderRadius:0,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px",
                ...
                <div style={{display:"flex",gap:10,padding:"16px 24px",
                  ...
                  <div style={{flex:1,background:"#fff",border: isPending ? "1px solid #fde68a" : "1px solid #bbf7d0",borderRadius:0,padding:"10px 14px",textAlign:"center"}}>
                  ...
                  <div style={{flex:1,background:"#fff",border: isPending ? "1px solid #fde68a" : "1px solid #bbf7d0",borderRadius:0,padding:"10px 14px",textAlign:"center"}}>
                    ...
                    <div style={{fontSize:13,fontWeight:700,color: isPending ? "#dc2626" : "#16a34a",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
                      <span style={{width:7,height:7,borderRadius:0,display:"inline-block",background: isPending ? "#f59e0b" : "#16a34a"}}/>
                ...
                <div style={{padding:"20px 24px"}}>
                  ...
                  <div style={{background:"#f8fafc",border:"1.5px solid #e2e8f0",borderRadius:0,padding:"11px 14px",fontSize:12,color:"#475569",wordBreak:"break-all",display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
                  ...
                  <div style={{
                    ...
                    borderRadius:0,padding:"10px 14px",fontSize:12,
                  ...
                  <div style={{display:"flex",gap:10,flexDirection:"column"}}>
                    <button onClick={()=>{
                      ...
                      color:"#fff",border:"none",padding:"13px",borderRadius:0,fontWeight:700,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,
                    ...
                    <button onClick={()=>{setShowLinkModal(false);onClose();showToast(isPending ? "Enrollment saved with pending payment." : "Enrollment complete!","success");}} style={{background:"#f1f5f9",color:"#475569",border:"1px solid #e2e8f0",padding:"11px",borderRadius:0,fontWeight:600,fontSize:13,cursor:"pointer"}}>
          ...
          <form id="purchase-form" className="purchase-form" onSubmit={submit} style={{display:"flex",gap:24,flexWrap:"wrap"}}>
            ...
            <div className="purchase-col" style={{flex:"1 1 320px",minWidth:0,display:"flex",flexDirection:"column",gap:16}}>
                      ...
                      <div className="stu-drop">
                        <div style={{padding:"8px 12px",borderBottom:"1px solid #f1f5f9"}}>
                          <input autoFocus value={stuSearch} onChange={e=>setStuSearch(e.target.value)} placeholder="Search..."
                            style={{width:"100%",border:"1.5px solid #e2e8f0",borderRadius:0,padding:"8px 12px",fontSize:13,outline:"none",boxSizing:"border-box"}}/>
                        </div>
              ...
              <div className="cp-card" style={{margin:0}}>
                ...
                {isInst&&(
                  <div style={{padding:"16px",background:"#eff6ff",borderRadius:0,border:"1px solid #bfdbfe",marginBottom:14}}>
            ...
            <div className="purchase-col" style={{flex:"1 1 320px",minWidth:0,display:"flex",flexDirection:"column",gap:16}}>
              ...
              <div style={{background:"linear-gradient(135deg,#eef2ff,#e0e7ff)",borderRadius:0,border:"1px solid #c7d2fe",padding:"20px 22px",marginTop:"auto"}}>
    ...
    <div className="manager-content">
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",backdropFilter:"blur(4px)",zIndex:9999,display:"flex",alignItems:"flex-start",justifyContent:"center",animation:"fadeIn .2s",overflowY:"auto",padding:"40px 16px"}}>
          <div style={{background:"#fff",width:"100%",maxWidth:400,borderRadius:0,boxShadow:"0 20px 25px -5px rgba(0,0,0,.1)",overflow:"hidden",animation:"slideUp .3s ease-out"}}>
            ...
            <div style={{padding:24,display:"flex",gap:12}}>
              <button onClick={()=>setDeleteId(null)} disabled={deleting} style={{flex:1,padding:"10px",borderRadius:0,border:"1.5px solid #e2e8f0",background:"#fff",color:"#475569",fontWeight:700,fontSize:14,cursor:"pointer"}}>Cancel</button>
              <button onClick={deletePurchase} disabled={deleting} style={{flex:1,padding:"10px",borderRadius:0,border:"none",background:"#ef4444",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
      ...
      <header style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,flexWrap:"wrap",gap:12}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={exportCSV} style={{background:"#f1f5f9",color:"#475569",border:"1px solid #e2e8f0",padding:"10px 16px",borderRadius:0,fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:8}}>
            <Icon name="download" size={14}/> Export CSV
          </button>
          <button onClick={()=>setShowModal(true)} style={{background:"#0f172a",color:"#fff",border:"none",padding:"10px 20px",borderRadius:0,fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:8}}>
      ...
      {overdueList.length>0&&(
        <div style={{background:"#fef2f2",border:"1px solid #fecaca",borderRadius:0,padding:"16px 20px",marginBottom:20,display:"flex",gap:16,alignItems:"flex-start"}}>
          <div style={{width:40,height:40,borderRadius:0,background:"#fee2e2",color:"#ef4444",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon name="alert-triangle" size={20}/></div>
          <div style={{flex:1}}>
            ...
            <div style={{display:"flex",gap:10,overflowX:"auto",paddingBottom:8}}>
              {overdueList.map(o=>(
                <div key={o.purchase_id} onClick={()=>setDetailId(o.purchase_id)} style={{background:"#fff",border:"1px solid #fca5a5",borderRadius:0,padding:"10px 14px",minWidth:220,cursor:"pointer",boxShadow:"0 2px 4px rgba(239,68,68,0.1)"}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",marginTop:8,alignItems:"center"}}>
                    ...
                    <div style={{fontSize:10,background:"#fee2e2",color:"#ef4444",padding:"2px 6px",borderRadius:0,fontWeight:700}}>Due: {o.due_date}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Filters */}
      <div style={{display:"flex",gap:12,marginBottom:20,background:"#fff",padding:"16px 20px",borderRadius:0,border:"1px solid #e2e8f0",boxShadow:"0 1px 3px rgba(0,0,0,.04)",alignItems:"center",flexWrap:"wrap"}}>
        <div style={{flex:"1 1 200px",position:"relative"}}>
          <div style={{position:"absolute",left:14,top:10,color:"#94a3b8"}}><Icon name="search" size={16}/></div>
          <input type="text" placeholder="Search by student name or email..." value={search} onChange={e=>setSearch(e.target.value)} style={{width:"100%",boxSizing:"border-box",padding:"10px 14px 10px 40px",borderRadius:0,border:"1px solid #e2e8f0",outline:"none",fontSize:13,color:"#0f172a"}}/>
        </div>
        <select value={filterCourse} onChange={e=>setFilterCourse(e.target.value)} style={{padding:"10px 14px",borderRadius:0,border:"1px solid #e2e8f0",outline:"none",fontSize:13,color:"#0f172a",background:"#fff",cursor:"pointer",minWidth:180}}>
          <option value="all">All Courses</option>
          {courses.map((c:any)=><option key={c.id} value={c.id.toString()}>{c.title}</option>)}
        </select>
        <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={{padding:"10px 14px",borderRadius:0,border:"1px solid #e2e8f0",outline:"none",fontSize:13,color:"#0f172a",background:"#fff",cursor:"pointer",minWidth:140}}>
      ...
      <div style={{background:"#fff",borderRadius:0,boxShadow:"0 1px 3px rgba(0,0,0,.06)",border:"1px solid #f1f5f9",overflow:"hidden"}}>
        ...
        <div style={{overflowX:"auto"}}>
          ...
          {loadingPurchases?(
            ...
            <div style={{padding:"60px",textAlign:"center"}}>
              <div style={{width:56,height:56,borderRadius:0,background:"#f1f5f9",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",color:"#94a3b8"}}><Icon name="shopping-cart" size={26}/></div>
              ...
              <button onClick={()=>setShowModal(true)} style={{background:"#6366f1",color:"#fff",border:"none",padding:"12px 24px",borderRadius:0,fontSize:14,fontWeight:700,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:8}}>
            ...
            return(
              <div key={p.id} className="tbl-row" style={{gridTemplateColumns:COLS,minWidth:960,cursor:"pointer",opacity:p.is_active?1:0.6,background:p.is_active?"#fff":"#f8fafc"}} onClick={()=>p.is_installment?setDetailId(p.id):setGenDetailId(p.id)}>
                ...
                <div>
                  {p.is_installment?(
                    <span style={{background:"#f1f5f9",padding:"4px 10px",borderRadius:0,fontSize:11,fontWeight:700,color:"#475569",display:"inline-flex",alignItems:"center",gap:4}}>
                      <Icon name="list" size={12}/> {p.installments_paid}/{p.total_installments} Paid
                    </span>
                  ):(
                    <span style={{background:"#f8fafc",border:"1px solid #e2e8f0",padding:"4px 10px",borderRadius:0,fontSize:11,fontWeight:700,color:"#64748b",display:"inline-flex",alignItems:"center",gap:4}}>
        ...
        {filteredPurchases.length>itemsPerPage&&(
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 20px",background:"#fff",borderTop:"1px solid #f1f5f9",flexWrap:"wrap",gap:10}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button disabled={currentPage===1} onClick={()=>setCurrentPage(currentPage-1)} style={{padding:"8px 14px",borderRadius:0,border:"1px solid #e2e8f0",background:"#fff",color:"#0f172a",fontSize:13,fontWeight:600,cursor:currentPage===1?"not-allowed":"pointer",opacity:currentPage===1?.5:1}}>Previous</button>
              <button disabled={currentPage===Math.ceil(filteredPurchases.length/itemsPerPage)} onClick={()=>setCurrentPage(currentPage+1)} style={{padding:"8px 14px",borderRadius:0,border:"1px solid #e2e8f0",background:"#fff",color:"#0f172a",fontSize:13,fontWeight:600,cursor:currentPage===Math.ceil(filteredPurchases.length/itemsPerPage)?"not-allowed":"pointer",opacity:currentPage===Math.ceil(filteredPurchases.length/itemsPerPage)?.5:1}}>Next</button>
            </div>
          </div>
        )}
      </div>
