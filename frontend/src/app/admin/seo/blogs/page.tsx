...
function SeoBlogsInner() {
  ...
  const load = async () => {
    try {
      const res = await apiFetch(`${BASE_URL}/api/blogs?limit=500`);
      if (res.ok) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await apiFetch(`${BASE_URL}/api/blogs/${editing.id}`, {
    ...
    <div className="manager-content" style={{ width: "100%" }}>
      <header style={{ marginBottom: 20 }}>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: "#0f172a", display: "flex", alignItems: "center", gap: 10 }}>
          <Icon name="edit" size={24} /> Blog SEO Manager
        </h1>
        <p style={{ margin: "6px 0 0", fontSize: 13, color: "#64748b" }}>Manage SEO titles, descriptions, and keywords for blog posts.</p>
      </header>

      <input style={{ ...S.input, marginBottom: 16 }} placeholder="Search blog posts..." value={search} onChange={(e) => setSearch(e.target.value)} />

      <SeoTable
        columns={[
          { key: "title", label: "Post", render: (r) => <span style={{ fontWeight: 600 }}>{r.title}</span> },
          ...
          { key: "actions", label: "", render: (r) => <button onClick={(e) => { e.stopPropagation(); handleEdit(r); }} style={{ padding: "6px 14px", border: "1px solid #e2e8f0", background: "#fff", cursor: "pointer", fontSize: 12, color: "#475569" }}>Edit SEO</button> },
        ]}
        data={filtered}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }} onClick={() => setEditing(null)}>
          <div style={{ background: "#fff", maxWidth: 600, width: "100%", maxHeight: "90vh", overflow: "auto", padding: 28 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <input style={S.input} value={form.seo_keywords} onChange={(e) => setForm({ ...form, seo_keywords: e.target.value })} placeholder="ai, programming, tutorial" />
            </div>
...
export default function SeoBlogsPage() {
  return <AdminProvider><SeoBlogsInner /></AdminProvider>;
}
