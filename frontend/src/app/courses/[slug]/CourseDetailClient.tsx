...
function VideoDropzoneField({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      const result = await uploadWithProgress(uploadEndpoint, formData, (pct) => setUploadProgress(pct));
      if (result.ok) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 10, color: "#64748b", marginTop: 6, fontWeight: 600 }}>
        {uploading ? (uploadProgress > 0 ? `Uploading... ${uploadProgress}%` : "Uploading, please wait...") : showUploadOnly ? "Click Upload to select a file" : "Paste a URL or drag & drop a file here"}
      </div>
    </div>
  );
}
...
export default function CourseDetailsPage({ slug: propSlug, initialData }: { slug?: string; initialData?: any }) {
  const { slug: paramSlug } = useParams();
  const slug = propSlug || paramSlug;
  const [course, setCourse] = useState<CourseDetails | null>(initialData || null);
  ...
  const [r2PickerOpen, setR2PickerOpen] = useState<number | null>(null);
  const [bannerUploading, setBannerUploading] = useState(false);
  const [bannerUploadProgress, setBannerUploadProgress] = useState(0);

  // Tools Covered Editor State
  const [toolsEditorOpen, setToolsEditorOpen] = useState(false);
  const [toolsForm, setToolsForm] = useState({
    section_eyebrow: "",
    section_title: "",
    section_desc: "",
    tools: [] as Array<{ name: string; icon_url: string }>
  });
  const [draggedToolIdx, setDraggedToolIdx] = useState<number | null>(null);
  const [dragOverToolIdx, setDragOverToolIdx] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    // If initialData was passed from the server wrapper, use it directly and skip the fetch.
    // This makes the course content available in the initial HTML for SEO/crawlers.
    if (initialData) {
      setCourse(initialData);
      if (initialData.chapters && initialData.chapters.length > 0) {
        setOpenChapters({ [initialData.chapters[0].id]: true });
      }
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="cd-root">
          ...
          <div className="cd-container cd-hero-layout">
            ...
            <div className="cd-hero-card">
              ...
              <div className="cd-hero-card-body">
                ...
                <button onClick={() => setSlotBookingOpen(true)} className="cd-btn-primary" style={{ width: "100%", justifyContent: "center" }}>
                    ...
                    {videoPlaylist.videos.map((v: any, i: number) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={() => { setActiveVideoIdx(i); setVideoUserClicked(true); }}
                        >
      ...
      {/* ── TOOLS COVERED MARQUEE (left-to-right) ── */}
      <section className="cd-tools-sec">
        {isAdmin && (
          <button onClick={() => {
            const tc = course?.extended?.tools_covered || {};
            setToolsForm({
              section_eyebrow: tc.section_eyebrow || "Tools & Technologies",
              section_title: tc.section_title || "Tools Covered in course",
              section_desc: tc.section_desc || "Hands-on mastery over the industry's leading AI tools, frameworks, and platforms.",
              tools: (tc.tools || []).map((t: any) => ({ name: t.name || "", icon_url: t.icon_url || "" }))
            });
            setToolsEditorOpen(true);
          }} className="cd-admin-edit-btn">
            <Icons.Edit /> Edit Tools Covered
          </button>
        )}
        <div className="cd-tools-title">{toolsCovered.section_title || "Tools Covered in course"}</div>
        <div className="cd-tools-marquee">
          <div className="cd-tools-track">
            {toolsCovered.tools && toolsCovered.tools.length > 0
              ? [...toolsCovered.tools, ...toolsCovered.tools].map((t: any, idx: number) => {
                  const iconSrc = t.icon_url && t.icon_url.startsWith("http") ? t.icon_url : (t.icon_url ? `${baseUrl}${t.icon_url}` : "");
                  return (
                    <div key={`tool-${idx}`} className="cd-partner-logo-cell" title={t.name || ""}>
                      {iconSrc ? (
                        <Image
                          src={iconSrc}
                          alt={t.name || "Tool icon"}
                          width={120}
                          height={44}
                          unoptimized
                          className="cd-partner-logo-img"
                        ...
                        <div className="cd-partner-pill">
                          <Icons.Sparkles /> <span>{t.name || "Tool"}</span>
                        </div>
                      )}
                    </div>
                  );
                })
              : [...DEFAULT_TOOLS, ...DEFAULT_TOOLS].map((t: any, idx: number) => (
                  <div key={`tool-default-${idx}`} className="cd-partner-pill">
                    <Icons.Sparkles /> <span>{t.name}</span>
                  </div>
                ))
            }
          </div>
          ...
          <div className="cd-hero-editor-body">
            <div className="cd-hero-editor-form">
              ...
              <div className="cd-hero-editor-group">
                ...
                />
                {/* Upload as Banner — saves to R2 + AboutBanner table */}
                <div style={{ marginTop: 8, display: "flex", gap: 8, alignItems: "center" }}>
                  <label style={{
                    padding: "6px 12px", borderRadius: 6, fontSize: 11, fontWeight: 600,
                    border: "1.5px solid #e63946", background: "rgba(230,57,70,0.08)", color: "#e63946",
                    cursor: bannerUploading ? "wait" : "pointer", transition: "all 0.15s",
                    display: "inline-flex", alignItems: "center", gap: 5,
                  }}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2" /><circle cx="8.5" cy="8.5" r="1.5" /><polyline points="21,15 16,10 5,21" /></svg>
                    {bannerUploading ? (bannerUploadProgress > 0 ? `${bannerUploadProgress}%` : "Uploading...") : "Upload as Banner"}
                    <input
                      type="file"
                      accept="image/*"
                      style={{ display: "none" }}
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          const result = await uploadWithProgress(
                            ...
                            fd,
                            (pct) => setBannerUploadProgress(pct)
                          );
                          if (result.ok) {
                            setVideoPlaylistForm(p => ({ ...p, bg_image_url: result.data.image_url }));
                          } else {
                            alert("Banner upload failed: " + (result.error || "Unknown error"));
                          }
                        } catch {
                          alert("Network error. Could not upload banner.");
                        } finally {
                  ...
                  <span style={{ fontSize: 10, color: "#94a3b8" }}>Also saves to About Us banner carousel</span>
                </div>
              </div>
                ...
                {videoPlaylistForm.videos.map((v, i) => (
                  ...
                  >
                    ...
                    <div className="cd-form-group" style={{ marginBottom: 8 }}>
                      ...
                      <VideoDropzoneField
                        ...
                        onChange={val => {
                          ...
                          if (v.video_type === "youtube") {
                            ...
                            if (autoThumb) {
                              setYtFetchingIdx(i);
                              setYtFetchedIdx(prev => { const s = new Set(prev); s.delete(i); return s; });
                              fetch(`${API_BASE_URL}/youtube/info?url=${encodeURIComponent(val)}`)
                                ...
                                .then(info => {
                                  if (info) {
                                    ...
                                    setYtFetchedIdx(prev => { const s = new Set(prev); s.add(i); return s; });
                                  }
                                })
                                .catch(() => {})
                      ...
                      />
                      {/* R2 Library Search button */}
                      {v.video_type === "r2" && (
                        <button
                          type="button"
                          onClick={() => setR2PickerOpen(i)}
                          style={{
                            marginTop: 6, padding: "6px 12px", borderRadius: 6, fontSize: 11, fontWeight: 600,
                            border: "1.5px solid #6366f1", background: "rgba(99,102,241,0.08)", color: "#6366f1",
                            cursor: "pointer", transition: "all 0.15s", display: "inline-flex", alignItems: "center", gap: 5,
                          }}
                        >
                      ...
                      {v.video_type === "youtube" && v.video_url && ytFetchingIdx !== i && ytFetchedIdx.has(i) && (
            ...
            <div className="cd-hero-editor-preview">
              <div className="cd-hero-editor-preview-inner" style={{ maxWidth: 560, padding: 0, border: "none", background: "transparent" }}>
                ...
                {(() => {
                  ...
                  const firstThumb = firstVideo.thumbnail_url && firstVideo.thumbnail_url.startsWith("http") ? firstVideo.thumbnail_url : (firstVideo.thumbnail_url ? `${baseUrl}${firstVideo.thumbnail_url}` : "");
                  return (
          ...
          <div className="cd-hero-editor-header">
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <button onClick={async () => {
                ...
                try {
                  const payload = JSON.stringify({
                    section_eyebrow: toolsForm.section_eyebrow,
                    section_title: toolsForm.section_title,
                    section_desc: toolsForm.section_desc,
                    tools: toolsForm.tools.filter(t => t.name.trim() || t.icon_url.trim())
                  });
                  const res = await apiFetch(`/api/courses/${course.id}/extended`, {
                    method: "PUT",
                    body: JSON.stringify({ tools_covered_json: payload })
                  });
                  if (!res.ok) throw new Error("Failed to save");
                  setCourse(prev => {
                    if (!prev) return prev;
                    return { ...prev, extended: { ...prev.extended, tools_covered: JSON.parse(payload) } };
                  });
                  setSavingEdit(false);
                  setToolsEditorOpen(false);
                  showToast("Tools Covered section updated!");
                } catch (err: any) {
                  setSavingEdit(false);
                  showToast("Save error: " + err.message);
                }
            ...
            <div className="cd-hero-editor-form">
              ...
              <div className="cd-hero-editor-group">
                ...
                <div className="cd-form-group">
                  ...
                  <input
                    className="cd-form-input"
                    value={toolsForm.section_eyebrow}
                    onChange={e => setToolsForm(p => ({ ...p, section_eyebrow: e.target.value }))}
                ...
                <div className="cd-form-group">
                  ...
                  <input
                    className="cd-form-input"
                    value={toolsForm.section_title}
                    onChange={e => setToolsForm(p => ({ ...p, section_title: e.target.value }))}
                    placeholder="e.g. Tools Covered in course"
                  />
                </div>
                <div className="cd-form-group">
                  <label className="cd-form-label">Section Description</label>
                  <input
                    className="cd-form-input"
                    value={toolsForm.section_desc}
                    onChange={e => setToolsForm(p => ({ ...p, section_desc: e.target.value }))}
              ...
              <div className="cd-hero-editor-group">
                ...
                {toolsForm.tools.map((t, i) => (
                  <div
                    key={i}
                    draggable
                    onDragStart={() => setDraggedToolIdx(i)}
                    onDragOver={e => { e.preventDefault(); setDragOverToolIdx(i); }}
                    onDragLeave={() => setDragOverToolIdx(null)}
                    onDrop={() => {
                      if (draggedToolIdx === null || draggedToolIdx === i) { setDraggedToolIdx(null); setDragOverToolIdx(null); return; }
                      const updated = [...toolsForm.tools];
                      const [moved] = updated.splice(draggedToolIdx, 1);
                      updated.splice(i, 0, moved);
                      setToolsForm(p => ({ ...p, tools: updated }));
                      setDraggedToolIdx(null);
                      setDragOverToolIdx(null);
                    }}
                    onDragEnd={() => { setDraggedToolIdx(null); setDragOverToolIdx(null); }}
                    style={{
                      marginBottom: 12,
                      padding: 12,
                      borderRadius: 8,
                      border: `1px solid ${dragOverToolIdx === i ? "#38bdf8" : "rgba(255,255,255,0.08)"}`,
                      background: dragOverToolIdx === i ? "rgba(56,189,248,0.08)" : draggedToolIdx === i ? "rgba(255,255,255,0.03)" : "rgba(255,255,255,0.02)",
                      transition: "border-color 0.15s, background 0.15s",
                      opacity: draggedToolIdx === i ? 0.5 : 1,
                      cursor: "grab",
                    }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                      <div style={{ color: "#64748b", cursor: "grab", flexShrink: 0 }}>
                        <span style={{ fontSize: 14, lineHeight: 1 }}>⠿</span>
                      </div>
                      <span style={{ fontSize: 11, fontWeight: 700, color: "#64748b", flexShrink: 0, minWidth: 20 }}>#{i + 1}</span>
                      {(() => {
                        const icon = t.icon_url && t.icon_url.startsWith("http") ? t.icon_url : (t.icon_url ? `${baseUrl}${t.icon_url}` : "");
                        return icon ? (
                          <img src={icon} alt="" style={{ width: 40, height: 28, borderRadius: 5, objectFit: "contain", background: "#fff", padding: 2, flexShrink: 0 }} />
                        ) : (
                          <div style={{ width: 40, height: 28, borderRadius: 3, background: "#0a1628", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", color: "#475569" }}>
                            <Icons.Sparkles />
                          </div>
                        );
                      })()}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      <div>
                        <label className="cd-form-label" style={{ fontSize: 9 }}>Tool Name</label>
                        <input
                          className="cd-form-input"
                          style={{ padding: "8px 10px", fontSize: 13 }}
                          value={t.name}
                      ...
                      <div>
                        <label className="cd-form-label" style={{ fontSize: 9 }}>Icon Image</label>
                        <VideoDropzoneField
                          label=""
                          value={t.icon_url}
                          onChange={val => {
            ...
            <div className="cd-hero-editor-preview">
              <div className="cd-hero-editor-preview-inner" style={{ maxWidth: 560, padding: 0, border: "none", background: "transparent" }}>
                <div style={{ textAlign: "center", marginBottom: 20 }}>
                  ...
                  <h3 style={{ fontSize: 22, fontWeight: 600, color: "#f1f5f9", margin: "0 0 8px" }}>
                    {toolsForm.section_title || "Tools Covered in course"}
                  </h3>
                  <p style={{ fontSize: 13, color: "#94a3b8", margin: "0 0 18px" }}>
                    {toolsForm.section_desc || "Hands-on mastery over the industry's leading AI tools..."}
                  </p>
                </div>
                {(() => {
                  const validTools = toolsForm.tools.filter(t => t.name.trim() || t.icon_url.trim());
                  if (validTools.length === 0) {
                    return (
                      <div style={{ textAlign: "center", padding: 40, color: "#94a3b8", fontSize: 13, background: "#f8fafc", borderRadius: 12 }}>
                        No tools added yet. Add a tool to see preview.
                      </div>
                    );
                  }
                  return (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "center" }}>
                      {validTools.slice(0, 8).map((t, i) => {
                        const icon = t.icon_url && t.icon_url.startsWith("http") ? t.icon_url : (t.icon_url ? `${baseUrl}${t.icon_url}` : "");
                        return icon ? (
                          <div key={i} style={{ width: 80, height: 44, background: "#fff", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", padding: 6 }}>
                            <img src={icon} alt={t.name} style={{ maxWidth: "100%", maxHeight: "100%", objectFit: "contain" }} />
                          </div>
                        ) : (
                          <div key={i} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", padding: "8px 16px", borderRadius: 30, fontSize: 12, fontWeight: 600, color: "#1e293b", display: "inline-flex", alignItems: "center", gap: 6 }}>
                            <Icons.Sparkles /> {t.name || "Tool"}
                          </div>
                        );
                      })}
                    </div>
      ...
      {/* R2 Video Library Picker */}
      {r2PickerOpen !== null && (
        <R2VideoPicker
          onClose={() => setR2PickerOpen(null)}
          onSelect={(material) => {
            const i = r2PickerOpen;
            setVideoPlaylistForm(prev => {
              const vids = [...prev.videos];
              vids[i] = {
                ...vids[i],
